...
const CoinList = (props: { data: CoinType[]; currency: string; viewType: string }) => {
  ...
  const columns: TableProps<any>['columns'] = [
    {
      ...
      render: (value: boolean, item: CoinType, index: number) => (
        <span
          className={value ? 'text-yellow-200 cursor-pointer' : 'cursor-pointer'}
          onClick={() => {
    ...
    },
    {
      ...
      render: (value) => {
        return {
          ...
          children: <div>{value && value.toLocaleString('ko-KR', { maximumFractionDigits: 2 })}%</div>,
        };
      },
    },
    {
  ...
  const onChangeBookMark = (value: boolean, item: CoinType, index: number) => {
    if (props.viewType === Constant.VIEW_TYPE.ALL) {
      // 전체보기
      const changedItem = dataSource.filter((data: CoinType) => data.id === item.id)[0];
      ...
      if (!value) {
        ...
        bookmarkArr = bookmarkArr.filter((bookmarkItem: CoinType) => bookmarkItem.id !== item.id);
        localStorage.setItem(
          'bookmark',
  ...
  return (
    <>
      <Table className="pt-3" columns={columns} dataSource={dataSource} pagination={false} scroll={{ y: 500 }} />
      {contextHolder}
    </>
  );
};
